...
public class DataUtil {
    ...
    /**
     * 计算年龄：大于1的，取整，得到xxx岁；小于1的，乘以12，取整，得到xxx个月
     */
    public static String ConvertAge2Str(Double age_f) {
        Integer age_i = 0;
        String ageStr = "未知";
        if (age_f == null) {
            age_f = Double.valueOf(0);
        }
        if (age_f >= 1.0f) {
            age_i = age_f.intValue();
            ageStr = age_i + "周岁";
        } else {
            Double age_tmp = age_f * 12.0f;
            age_i = age_tmp.intValue();
            ageStr = age_i + "个月";
        }
     ...
     * 对MessageDialog再封装一层
     */
    public static void showNormalDialog(Context context, String title, String message, String positive, String negative, OnNormalDialogClicker onNormalDialogClicker) {
