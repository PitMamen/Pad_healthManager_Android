...
import com.bitvalue.healthmanage.aop.CheckNet;
...
import com.bitvalue.healthmanage.ui.adapter.RecyclerPagerAdapter;
...
public final class ImagePreviewActivity extends AppActivity
    ...
    public static void start(Context context, List<String> urls, int index) {
        ...
        if (urls instanceof ArrayList) {
            intent.putExtra(Constants.IMAGE, (ArrayList<String>) urls);
        } else {
            intent.putExtra(Constants.IMAGE, new ArrayList<>(urls));
        }
    ...
    protected void initData() {
        ArrayList<String> images = getStringArrayList(Constants.IMAGE);
        ...
        if (images.size() != 1) {
            ...
            int index = getInt(Constants.INDEX);
