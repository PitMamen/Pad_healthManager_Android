...
public class NewMsgFragment extends AppFragment implements BGANinePhotoLayout.Delegate {
    ...
    BGANinePhotoLayout ninePhotoLayout;   //九宫格图片控件
...
//    RecyclerView list_photos;
    ...
    private List<UploadFileApi> mUploadedAudios = new ArrayList<>(); //录音缓存
    ...
    private ArrayList<String> photosFinal = new ArrayList<>();  //存放图片文件
    private ArrayList<String> audiosFinal = new ArrayList<>();  //存放音频文件
    private ArrayList<String> articles = new ArrayList<>();    //存放文章文件
...
//                            录音
                            AudioPlayer.getInstance().startRecord(success -> {
                                ...
                                String path = AudioPlayer.getInstance().getPath();  //录音存放地址
                                //组装录音列表数据
