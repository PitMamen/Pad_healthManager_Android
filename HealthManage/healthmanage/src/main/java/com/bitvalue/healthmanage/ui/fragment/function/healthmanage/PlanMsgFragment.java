...
import com.bitvalue.healthmanage.http.api.GetCustomMsgApi;
 ...
 * 健康提醒界面
 */
public class PlanMsgFragment extends AppFragment implements BGANinePhotoLayout.Delegate {
    ...
    private TaskPlanDetailBean taskPlanDetailBean;

    @Override
    protected int getLayoutId() {
        return R.layout.fragment_plan_msg;
    }
...
//        msgCustomId = getArguments().getString(Constants.MSG_CUSTOM_ID);
        ...
        taskPlanDetailBean = (TaskPlanDetailBean) getArguments().getSerializable(Constants.PLAN_MSG);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.img_add_pic:
                PermissionUtils.checkPermission(getAttachActivity(), new PermissionUtils.PermissionCallBack() {
                    @Override
...
//        }
    ...
    private void uploadedAudioMsgs() {
        FileUploadUtils.INSTANCE.uploadAudio(PlanMsgFragment.this, mUploadedAudios.get(i[0]), new FileUploadUtils.OnAudioUploadCallback() {
            @Override
    ...
    private void uploadedPicMsgs() {
        FileUploadUtils.INSTANCE.uploadPic(PlanMsgFragment.this, mUploadImages.get(j[0]), new FileUploadUtils.OnAudioUploadCallback() {
            @Override
            public void onSuccess(ApiResult<AudioUploadResultBean> result) {
