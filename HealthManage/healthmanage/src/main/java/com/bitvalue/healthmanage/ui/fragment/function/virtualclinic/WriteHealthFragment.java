...
import com.bitvalue.healthmanage.http.api.GetHistoryApi;
import com.bitvalue.healthmanage.http.api.SaveCaseApi;
...
//                病例预览
            case R.id.tv_preview:
                ...
                homeActivity.switchSecondFragment(Constants.FRAGMENT_HEALTH_HISTORY_PREVIEW, msgData);//跳转至云门诊病例
                break;
                //提交病例点击
            case R.id.tv_commit:
                if (ifReady()) {
...
//请求接口 查询病历
    private void getCaseData() {
        ...
        EasyHttp.post(this).api(getHistoryApi).request(new HttpCallback<ApiResult<List<SaveCaseApi>>>(this) {
            ...
            @Override
            public void onSucceed(ApiResult<List<SaveCaseApi>> result) {
    ...
    private void commitCase() {
        ...
        EasyHttp.post(this).api(saveCaseApi).request(new HttpCallback<ApiResult<SaveCaseApi>>(this) {
            @Override
            public void onStart(Call call) {
                super.onStart(call);
            }
            ...
            @Override
            public void onSucceed(ApiResult<SaveCaseApi> result) {
                super.onSucceed(result);
                if (result.getCode() == 0) {
