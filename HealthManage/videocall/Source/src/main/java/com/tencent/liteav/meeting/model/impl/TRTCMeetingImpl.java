...
public class TRTCMeetingImpl extends TRTCMeeting implements ITXTRTCMeetingDelegate, ITXRoomServiceDelegate {
    ...
    @Override
    public void createMeeting(final String roomId, final TRTCMeetingCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
                clear();
                //IM 先创建房间
                mRoomId = roomId;
                TXRoomService.getInstance().createRoom(roomId, roomId, "", new TXCallback() {
                    @Override
                    public void onCallback(int code, String msg) {
                        if (code == 0) {
                            // IM 创建房间成功进入trtc房间
                            TXTRTCMeeting.getInstance().enterRoom(mSdkAppId, roomId, mUserId, mUserSig,
                                    new TXCallback() {
                                        @Override
                                        public void onCallback(int code, String msg) {
                                            if (callback != null) {
    ...
    @Override
    public void destroyMeeting(final TRTCMeetingCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
    ...
    @Override
    public void enterMeeting(final String roomId, final TRTCMeetingCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
                clear();
                mRoomId = roomId;
                TXRoomService.getInstance().enterRoom(roomId, new TXCallback() {
                    @Override
                    public void onCallback(int code, String msg) {
                    }
                });
    ...
    @Override
    public void setLocalViewFillMode(final int fillMode) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
                TXTRTCMeeting.getInstance().setLocalViewFillMode(fillMode);
            }
        });
    }
    ...
    @Override
    public void enableCustomVideoCapture(TRTCCloudListener.TRTCVideoRenderListener videoRenderListener) {
        TXTRTCMeeting.getInstance().enableCustomVideoCapture(videoRenderListener);
    }

    @Override
    public void disableCustomVideoCapture() {
        TXTRTCMeeting.getInstance().disableCustomVideoCapture();
    }

    @Override
    public void sendCustomVideoData(EGLContext eglContext, int textureId, int width, int height) {
        TXTRTCMeeting.getInstance().sendCustomVideoData(eglContext, textureId, width, height);
    }
}
