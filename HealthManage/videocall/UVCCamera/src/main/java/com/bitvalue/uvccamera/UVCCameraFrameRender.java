...
import com.bitvalue.uvccamera.model.Size;
import com.bitvalue.uvccamera.render.EglCore;
import com.bitvalue.uvccamera.render.opengl.GPUImageFilter;
import com.bitvalue.uvccamera.render.opengl.GpuImageI420Filter;
...
public class UVCCameraFrameRender implements TRTCCloudListener.TRTCVideoRenderListener, TRTCCloudListener.TRTCAudioFrameListener, Handler.Callback {
    public static final String TAG = UVCCameraFrameRender.class.getSimpleName();
    ...
    public void onRenderVideoFrame(String userId, int streamType, final TRTCCloudDef.TRTCVideoFrame frame) {
        if (!userId.equals(mUserId) || mSteamType != streamType) {
            Log.d(TAG, "onRenderVideoFrame: user or streamType not matched.");
            return;
        }
    ...
    public UVCCameraFrameRender(String userId, int steamType) {
        mUserId = userId;
        ...
        Log.i(TAG, "UVCCameraFrameRender created.");
    }
    ...
    public void start(SurfaceView surfaceView) {
        if (surfaceView == null) {
            Log.w(TAG, "start error when surface view is null");
            return;
        }

        Log.i(TAG, "start render with " + surfaceView);
    ...
    public void start(TextureView textureView) {
        ...
        Log.i(TAG, "start render with " + textureView);

        // 设置TextureView的SurfaceTexture生命周期回调，用于管理GLThread的创建和销毁
        mTextureView = textureView;
        mSurfaceTexture = mTextureView.getSurfaceTexture();
        Log.d(TAG, "start: mSurfaceTexture = " + mSurfaceTexture);
    ...
    public void stop() {
        if (mTextureView != null) {
            Log.d(TAG, "stop with textureView.");
            mTextureView.setSurfaceTextureListener(null);
        } else if (mSurfaceView != null) {
            Log.d(TAG, "stop with surfaceView");
            mSurfaceView.getHolder().removeCallback(mSurfaceHolderCallback);
        }
    ...
    public void setFillMode(boolean fill) {
        mScaleType = fill ? ScaleType.CENTER_CROP : ScaleType.FIT_CENTER;
    }

    private void initGlComponent(Object eglContext) {
        Log.d(TAG, "initGlComponent: entering.");
        ...
        if (mSurface != null) {
            ...
            Log.d(TAG, "initGlComponent: no mSurface or mSurfaceTexture specified.");
            return;
        }

        try {
            if (eglContext instanceof javax.microedition.khronos.egl.EGLContext) {
                mEglCore = new EglCore((javax.microedition.khronos.egl.EGLContext) eglContext, surface);
            } else {
                mEglCore = new EglCore((android.opengl.EGLContext) eglContext, surface);
            }
        } catch (Exception e) {
    ...
    private void renderInternal(TRTCCloudDef.TRTCVideoFrame frame) {
        ...
        if (mEglCore == null && (mSurface != null || mSurfaceTexture != null)) {
        ...
        if (mEglCore == null) {
            // Log.w(TAG, "renderInternal: not rendered due to mElgCore == null.");
            return;
        }
        ...
        if (mLastInputSize.width != frame.width || mLastInputSize.height != frame.height
            ...
            Pair<float[], float[]> cubeAndTextureBuffer = OpenGlUtils.calcCubeAndTextureBuffer(mScaleType,
            ...
            Log.d("XAVIER", "input size " + frame.width + "x" + frame.height  + ", output size " + mSurfaceSize.width + "x" + mSurfaceSize.height +
